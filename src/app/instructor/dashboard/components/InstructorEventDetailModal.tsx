...
const InstructorCard = styled.div<{ $role?: 'MAIN' | 'ASSISTANT' | 'MANAGER' }>`
  ...
  background: ${(props) => (props.$role === 'MAIN' ? '#eff6ff' : props.$role === 'MANAGER' ? '#f1f5f9' : '#f8fafc')};
  border: 1px solid ${(props) => (props.$role === 'MAIN' ? '#dbeafe' : props.$role === 'MANAGER' ? '#64748b' : '#f1f5f9')};

  .left { display: flex; align-items: center; gap: 12px; }
  .avatar {
    width: 32px; height: 32px; border-radius: 50%;
    background: ${(props) => (props.$role === 'MAIN' ? '#3b82f6' : props.$role === 'MANAGER' ? '#94a3b9' : '#cbd5e1')};
  ...
  .info {
    ...
    .role {
      ...
      color: ${(props) => (props.$role === 'MAIN' ? '#3b82f6' : props.$role === 'MANAGER' ? '#94a3b8' : '#94a3b8')};
    }
  }
...
const LECTURE_STATUS_MAP: Record<string, string> = {
  RECRUITING: '모집중', ALLOCATING: '배정중', COMPLETED: '배정완료'
};
...
export default function InstructorEventDetailModal({ event, onClose }: Props) {
  ...
  // 2. 상태 뱃지 설정
  const lectureStateRaw = (event.status as string) === 'CONFIRMED' ? 'COMPLETED' : (event.status as string) || 'RECRUITING';
  ...
  const showInstructorInfo = ['PENDING', 'CONFIRMED'].includes(event.instructorStatus);
  ...
  return createPortal(
    <Backdrop onClick={onClose}>
      <ModalBox onClick={(e) => e.stopPropagation()}>
        ...
        <Header>
          <TitleArea>
            <Badges>
              ...
              {/* 상태 뱃지 */}
              <Badge $bg={statusBg} $color={statusColor}>{displayStatus}</Badge>
            </Badges>
            <h2>{event.title}</h2>
          </TitleArea>
        ...
        {/* 담당자 및 강사 리스트 */}
        <Section>
          <SectionLabel><UsersIcon /> 담당자 및 강사</SectionLabel>
          ...
          {showInstructorInfo ? (
            <InstructorList>
              ...
              {managers.length === 0 && mainInstructors.length === 0 && assistInstructors.length === 0 && (
                <ContentBox style={{ textAlign: 'center', color: '#999' }}>배정된 인원이 없습니다.</ContentBox>
              )}
            </InstructorList>
          ) : (
