...
function getManagerName(m: Manager) {
  return m.name || m.username || `매니저 #${m.id}`;
}
...
export default function InstructorMessagesPage() {
    ...
    const fetchManagers = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const list: Manager[] = Array.isArray(data) ? data : data.results || [];
          setManagers(list);

          const initialCounts: Record<number, number> = {};
          list.forEach(m => {
            if (m.unread_count) initialCounts[m.id] = m.unread_count;
          });
  ...
  // 2. 매니저 선택 -> 히스토리 로드 & 뱃지 초기화
  useEffect(() => {
    if (!selectedManagerId) return;

    setUnreadCounts(prev => ({ ...prev, [selectedManagerId]: 0 }));

    const fetchHistory = async () => {
      setIsLoading(true);
      try {
        const res = await fetch(`${BACKEND_HTTP_BASE}/api/communications/history/${selectedManagerId}/`, {
          headers: createAuthHeaders() as any,
        });
        if (res.ok) {
          const data = await res.json();
          const sorted = (data.messages || []).sort((a: any, b: any) => new Date(a.sent_at).getTime() - new Date(b.sent_at).getTime());
          setMessages(sorted.map((m: any) => ({ ...m, isMine: m.sender_id !== selectedManagerId })));
        }
      } catch (e) { console.error(e); }
  ...
  useEffect(() => {
    ...
    const url = `${BACKEND_WS_BASE}/ws/chat/${selectedManagerId}/?token=${token}`;
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (data.event === "message") {
          ...
          if (msgEvent.sender_id === selectedManagerId || msgEvent.recipient_id === selectedManagerId) {
    ...
    <S.Wrap>
      <S.ChatContainer>
        ...
        <S.Sidebar>
          ...
          <S.UserList>
            {managers.map((mgr) => {
              const isActive = mgr.id === selectedManagerId;
              const name = getManagerName(mgr);
              const initial = name.charAt(0);
              const count = unreadCounts[mgr.id] || 0;

              return (
                <S.UserItem key={mgr.id} $active={isActive} onClick={() => setSelectedManagerId(mgr.id)}>
        ...
        <S.ChatArea>
          ...
          <S.ChatHeader>
            <div className="title-group">
              <span className="name">
                {currentManager ? getManagerName(currentManager) : "대화 상대 선택"}
              </span>
              {currentManager && (
          ...
          <S.MessageList>
            {!selectedManagerId ? (
              <S.EmptyState>
                <ChatIcon />
                <div>왼쪽 목록에서 매니저를 선택하여 대화를 시작하세요.</div>
              </S.EmptyState>
            ) : (
              <>
