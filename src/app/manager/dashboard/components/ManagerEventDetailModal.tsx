...
const Backdrop = styled.div`
  ...
  backdrop-filter: blur(4px); /* 배경 흐림 효과 (고급스러움) */
...
const TitleArea = styled.div`
  ...
  h2 {
    margin: 0; font-size: 1.5rem; font-weight: 800; color: #111;
    line-height: 1.3;
  }
`;
...
const InstructorCard = styled.div<{ $isMain?: boolean }>`
  ...
  .info {
    ...
    .name { font-weight: 700; color: ${(props) => props.$isMain ? '#1e293b' : '#475569'}; font-size: 0.95rem; }
    .role { font-size: 0.75rem; color: ${(props) => props.$isMain ? '#3b82f6' : '#94a3b8'}; font-weight: 600; }
  }
  .phone {
...
export default function ManagerEventDetailModal({ event, onClose }: Props) {
  ...
  const dateStr = `${startDate.getFullYear()}년 ${startDate.getMonth() + 1}월 ${startDate.getDate()}일`;
  const timeStr = `${String(startDate.getHours()).padStart(2, '0')}:${String(startDate.getMinutes()).padStart(2, '0')} ~ ${String(new Date(event.end).getHours()).padStart(2, '0')}:${String(new Date(event.end).getMinutes()).padStart(2, '0')}`;
    ...
    <Backdrop onClick={onClose}>
      <ModalBox onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          ...
          <Section>
            <SectionLabel><FileTextIcon /> 콘텐츠</SectionLabel>
            <ContentBox>{event.content || '-'}</ContentBox>
          </Section>
        </div>

        <Section>
          <SectionLabel><UsersIcon /> 담당 강사 ({event.instructors.length}명)</SectionLabel>
          <InstructorList>
            {/* 주강사 */}
            {mainInstructor ? (
              ...
              <ContentBox style={{ textAlign: 'center', color: '#999' }}>주강사 배정 전</ContentBox>
            )}
